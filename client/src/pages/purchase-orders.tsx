...
export default function PurchaseOrders() {
  ...
  const typedPaginatedItems: PurchaseOrder[] = paginatedItems;
              ...
              <div className="flex-1 overflow-auto scrollbar-hide">
                {loading ? (
                  ...
                  <div className="divide-y divide-slate-100 bg-white">
                    {typedPaginatedItems.map((po: PurchaseOrder) => (
                      <div
                        key={po.id}
                          ...
                          <thead className="bg-sidebar/5 dark:bg-sidebar/10 border-b border-slate-200 dark:border-slate-700">
                            <tr>
                              <th className="px-4 py-3 text-left text-[11px] font-bold text-sidebar/70 uppercase tracking-wider w-10 font-display">
                                <Checkbox
                                  checked={selectedPOs.length === typedPaginatedItems.length && typedPaginatedItems.length > 0}
                                  onCheckedChange={(checked) => {
                                    if (checked) {
                                      setSelectedPOs(typedPaginatedItems.map((po: PurchaseOrder) => po.id));
                                    } else {
                                      setSelectedPOs([]);
                                    }
                                  }}

...
import { Plus, Search, Filter, Landmark, Link2, ArrowLeft } from "lucide-react";
...
export default function Banking() {
  ...
  const mutation = useMutation({
    mutationFn: async (values: any) => {
      const res = await apiRequest("POST", "/api/bank-accounts", values);
      return res.json();
    },
    onSuccess: () => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {accounts.map((account: BankAccount) => (
            <Card key={account.id} className="hover-elevate cursor-pointer">
